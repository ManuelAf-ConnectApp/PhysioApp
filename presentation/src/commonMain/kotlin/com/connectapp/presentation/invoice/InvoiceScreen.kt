...
fun InvoiceScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                InvoiceEffect.NavigateBack -> onNavigateBack()
                InvoiceEffect.NavigateToCreatePatient -> onNavigateToCreatePatient()
                is InvoiceEffect.ShowError -> {
    ...
    if (state.isLoading) {
        ...
        Box(modifier = modifier) {
            InvoiceScreenContent(state, onIntent)
        }
    }
}
...
@Composable
fun InvoiceScreenContent(state: InvoiceState, onIntent: (InvoiceIntent) -> Unit) {
            ...
            confirmButton = {
                TextButton(onClick = {
                    datePickerState.selectedDateMillis?.let { millis ->
                        ...
                        onIntent(InvoiceIntent.DateChanged(dateString))
                    }
    ...
    Column(
        ...
        ExposedDropdownMenuBox(
            ...
            val selectedProfessionalName =
                state.professionals.find { it.id.toString() == state.professionalId }?.let {
            ...
            ExposedDropdownMenu(
                ...
                state.professionals.forEach { professional ->
                    DropdownMenuItem(
                        ...
                        onClick = {
                            onIntent(InvoiceIntent.ProfessionalSelected(professional.id.toString()))
        ...
        Box(modifier = Modifier.fillMaxWidth().zIndex(1f)) {
            Column {
                CustomOutlinedTextField(
                    ...
                    onValueChange = { onIntent(InvoiceIntent.PatientNameChanged(it)) },
                ...
                if (state.suggestedPatients.isNotEmpty()) {
                        ...
                        Column {
                            state.suggestedPatients.forEach { patient ->
                                Text(
                                    ...
                                    modifier = Modifier
                                        .fillMaxWidth()
                                        .clickable { onIntent(InvoiceIntent.PatientSelected(patient)) }
                                        .padding(DimensResources.spacing16),
                    ...
                    Card(
                        ...
                        Text(
                            ...
                            modifier = Modifier
                                .fillMaxWidth()
                                .clickable { onIntent(InvoiceIntent.AddPatientClicked) }
                                .padding(16.dp),
                            style = MaterialTheme.typography.bodyMedium,
                            color = MaterialTheme.colorScheme.primary,
                            fontWeight = FontWeight.Bold
        ...
        CustomOutlinedTextField(
            value = state.concept,
            onValueChange = { onIntent(InvoiceIntent.ConceptChanged(it)) },
        ...
        CustomOutlinedTextField(
            value = state.amount,
            onValueChange = { onIntent(InvoiceIntent.AmountChanged(it)) },
        ...
        Row(
            ...
            Switch(
                ...
                onCheckedChange = { onIntent(InvoiceIntent.PaidStatusChanged(it)) }
            )
        }

        Button(
            onClick = { onIntent(InvoiceIntent.SaveClicked) },
